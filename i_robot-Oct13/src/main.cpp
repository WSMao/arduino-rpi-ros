...
int mission4_control=0;
int mission5_control=0;
int mission6_control=0;
...
int main(int argc, char **argv)
{
  ...
  ros::Subscriber ir_sub = n.subscribe("ir_topic", 300, ir_Cb);  //topic and callback function
  ros::Subscriber imu_sub = n.subscribe("imu_topic", 300, imu_Cb);  //topic and callback function
  ros::Subscriber sonar_sub = n.subscribe("sonar_topic", 300, sonar_Cb);  //topic and callback function
  ros::Subscriber button_sub = n.subscribe("button_topic", 300, button_Cb);  //topic and callback function

  ros::Subscriber ros_mega2_sub = n.subscribe ("Mega2toRos_topic",1, Mega2Ros_Cb);

 // i_robot::ROS_MEGA2_msg mega2_message;
  ros::Publisher ros_mega2_pub = n.advertise<i_robot::ROS_MEGA2_msg> ("RostoMega2_topic",1);


  i_robot::arm_msg arm_message;
  arm_message.AI_degree[0]=150;
  ros::Publisher arm_pub   =n.advertise<i_robot::arm_msg> ("arm_topic",300);

  i_robot::motor_msg motor_message ;
  ros::Publisher motor_pub = n.advertise<i_robot::motor_msg> ("motor_topic", 300);

  i_robot::imu_msg imu_message ;
  ros::Publisher imu_pub = n.advertise<i_robot::imu_msg> ("imu_to_mega2", 300);
  //這裡睡可以 讓主程式跑慢點 等待其他裝置的訊息！
...
//           sleep(3);
           ...
           ros_mega2_pub.publish(ros_mega2);  //有時arduino會收不到
           //ros_mega2_pub.publish(ros_mega2);  //有時arduino會收不到
           //ros_mega2_pub.publish(ros_mega2);  //有時arduino會收不到
           sleep(1);//這裡會繼續直走 可能會太九
           ros_mega2_pub.publish(ros_mega2);  //有時arduino會收不到
           //ros_mega2_pub.publish(ros_mega2);  //有時arduino會收不到
           //ros_mega2_pub.publish(ros_mega2);  //有時arduino會收不到
           //usleep(2000000);
...
case 2:  //avoid and turn left!!
        ...
        if(sonar.sonar_front<=30 && sonar.sonar_front!=0 && mission2_control==0){//turn left
...
//            if(jump==1 && imu.yaw >0){//發生跳躍!   yaw>0 代表跳到另一區了
        ...
        if(mission2_control==2){//stop!!!
            cout<<"stop a second"<<endl;
            motor_message.pwm_l=0;
            motor_message.pwm_r=0;
            motor_pub.publish(motor_message);
//            ros::spinOnce();
...
case 3:
        ...
        //////////////
         if(mission3_control==2){  //turn now!!
 ...
 //            if(jump==1 && imu.yaw >0){//發生跳躍!   yaw>0 代表跳到另一區了
         ...
         ////////////
         if(mission3_control==3){
             std::cout<<"press the button!!"<<std::endl;
             arm_message.AI_degree[1]=200; //ID_2 235 to touch the button
         ...
         if(mission3_control==4){
             ...
             //sleep(1); //直走一秒
             usleep(500000);
             mission=4; //go to white_line_follower mission!!
             cout<< "go to white_line_follower mission" << endl;
         }
...
//       ir.an[0]=548;ir.an[1]=656;ir.an[2]=607;ir.an[3]=705;ir.an[4]=596;//這些值可能要供校正修改
//       ir.a1[0]=58;ir.a1[1]=72;ir.a1[2]=62;ir.a1[3]=74;ir.a1[4]=63;//這些值可能要供校正修改
    if(mission4_control==0){  //send the a1 and an to mega2 then start follow the white line in mega
        for(int i=0;i<5;i++){
             ros_mega2.a1[i]=ir.a1[i];
             ros_mega2.an[i]=ir.an[i];
        }
...
//           ros::spinOnce();
    ...
    //if(mission1_control==1){//wait the message from mega that it's the end of line follower!
    while(ros::ok() && mission4_control==1){
       // ros_mega2_pub.publish(ros_mega2);   //這邊發對方要收的到才行要剛好在聽
        std::cout<<"waitting the line_follower"<<std::endl;
        ros::spinOnce();//listen the message
        imu_message.yaw=imu.yaw;
        ...
        if(abs(0-imu.yaw)<10){
             ...
             std::cout<<"yes imu is almost 0 degree!!!"<<std::endl;
             imu_pub.publish(imu_message);
             usleep(500000);//spinOnce這裡一定要等待 似乎是接收arduino 資訊 怕漏接訊息
             ros::spinOnce();//listen the message
        }
...
case 5:
    std::cout<<"follow the wall then turn left!!" <<std::endl;
    if(mission5_control==0){//go straight
        cout<<"go straight"<<endl;
        cout<<"sonar_front: "<<sonar.sonar_front<<endl;
        motor_message.pwm_l=30;
        motor_message.pwm_r=30;
    ...
    if(mission5_control==1){  //follow the wall
        pwm=follow_wall(imu,sonar,0,35); // follow 0 degree and 22cm to the wall !
        ...
        if(sonar.sonar_front<=30 && sonar.sonar_front!=0 ){//turn left

                cout<<"turn left now"<<endl;
                dst=-90;//直接給絕對的-90度
                mission5_control=2; //switch to turn right section
        }
    }

    if(mission5_control==2){ //turn left
    ...
    if(mission5_control==3){//follow the wall again

        pwm=follow_wall(imu,sonar,-90,30); // follow -90 degree and 22cm to the wall !
    ...
    break;
case 6: //white line_follower
        std::cout<<"white line_follower now !!"<<std::endl;

        if(mission6_control==0){  //send the a1 and an to mega2 then start follow the white line in mega
            for(int i=0;i<5;i++){
                 ros_mega2.a1[i]=ir.a1[i];
                 ros_mega2.an[i]=ir.an[i];
            }
        ...
        while(ros::ok() && mission6_control==1){
           // ros_mega2_pub.publish(ros_mega2);   //這邊發對方要收的到才行要剛好在聽
            std::cout<<"waitting the line_follower"<<std::endl;
            ros::spinOnce();//listen the message
            imu_message.yaw=imu.yaw;
            ...
            if(ros_mega2.start_follow_white==0){ // if Mega2 tell the line_follower mission has finished
                 mission6_control=2;
                 mission=7;
                 std::cout<<"change to mission 7!!"<<std::endl;
                 std::cout<<"finish follow the line!"<<std::endl;
                 //sleep(1);
                 break;  //break while loop
            }
...
case 7: //black line_follower
         ...
         std::cout<<" black line_follower now !!"<<std::endl;
         ros_mega2.start_follow_black=1;
         ros_mega2_pub.publish(ros_mega2);  //有時arduino會收不到
         sleep(1);//這裡會繼續直走 可能會太九
         ros_mega2_pub.publish(ros_mega2);  //有時arduino會收不到
