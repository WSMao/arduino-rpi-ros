...
//    int an_ll=548,an_ml=656,an_mm=607,an_mr=705,an_rr=596;//這些值可能要供校正修改
//    int a1_ll=58,a1_ml=72,a1_mm=62,a1_mr=75,a1_rr=63;
    ...
    //if(!busy){select_mission(1,ir);}//
    if(!busy){  //如果沒有在執行特殊任務，則判斷任務

        if(cny70==0b11110 || cny70==0b11100 ){
        white_mode=WaitToTurnLeft;
        busy=1;
        wait=1;
        }

        else if(cny70==0b11000 || cny70==0b10000){
        white_mode=NowTurnLeft;
        busy=1;
        }

        else if(cny70==0b01111 || cny70==0b00111 ){
        white_mode=WaitToTurnRight;
        busy=1;
        wait=1;
        }

        else if(cny70==0b00111 || cny70==0b00001){
        white_mode=NowTurnRight;
        busy=1;
        }
        else{  white_mode =FuzzyMode; }


    }

    if(white_mode==WaitToTurnLeft){
        ...
        pwm.pwm_l= -60;//100
        pwm.pwm_r=60;//190
        if(ir.ir_digital[2]==1){white_mode=FuzzyMode;busy=0;}
        }
    }

    if(white_mode==WaitToTurnRight){
    ...
    if(white_mode==NowTurnLeft){
        std::cout<<"turning left"<<std::endl;
        pwm.pwm_l=-60;
        pwm.pwm_r=60;
        if(ir.ir_digital[2]==1){white_mode=FuzzyMode;busy=0;}
    }
    if(white_mode==NowTurnRight){
        std::cout<<"turning right"<<std::endl;
        pwm.pwm_l=60;
        pwm.pwm_r=-60;
        if(ir.ir_digital[2]==1){white_mode=FuzzyMode;busy=0;}
...
///////////////////////////////////////////////////FuzzyMode/////////////////////////////////////////////////
    if(white_mode==FuzzyMode){
          ...
          //算出每顆感測器的側偏差
            ...
            float new_t=ir.new_t;
            //std::cout<<"ir.new_t"<<ir.new_t<<std::endl;
...
//            std::cout<<"err_rate: "<<err_rate<<std::endl;
           // _sleep(300);
            //usleep(300);
/////////////////////////////////////////////////////////////White_Fuzzy///////////////////////////////////////////////
            float difference=White_Fuzzy( err_new , err_rate );
/////////////////////////////////////////////////////////////White_Fuzzy///////////////////////////////////////////////
          ...
          //計算後輸出轉速
            ...
            pwm.pwm_r=initial-difference;   /////////////////////////////////////1.5/////////////////////////////////
           //左右馬達前進輸出
...
///////////////////////////////////////////////////////////////////////////////////////Fuzzy//////////////////////////////////////////////////////////////////////////
float White_Fuzzy(float err_new, float err_rate)     //  模糊控制
{
  float difference;
  ...
  float NB=-100 , NM=-25 , NS=-15 , Z0=0 , PS=-NS , PM=-NM , PB=-NB ;
  float rule[8][8] = {
    //    車體偏右                                    車體偏左
    //區間1     2     3     4     5     6     7     8
    ...
    ,                                                                               // 1
    { 50 ,  NS ,  PS ,  PM ,  PM ,  PB ,  PB ,  PB  } //PB         往左變化
    ,                                                                               // 2
    { 10 ,  NM ,  Z0 ,  PS ,  PM ,  PB ,  PB ,  PB  } //PM
    ,                                                                               // 3
    {  5 ,  NB ,  NM ,  Z0 ,  Z0 ,  PM ,  PB ,  PB  } //PS
    ,                                                                               // 4
    {  0  ,  NB ,  NM ,  NS ,  Z0 ,  PM ,  PM ,  PB  } //Z0
    ,                                                                               // 5
    {  -5 ,  NB ,  NB ,  NM ,  Z0 ,  Z0 ,  PM ,  PB  } //NS
    ,                                                                               // 6
    {-10 ,  NB ,  NB ,  NB ,  NM ,  NS ,  Z0 ,  PB  } //NM
    ,                                                                              // 7
    {-50 ,  NB ,  NB ,  NB ,  NM ,  NM ,  NS ,  PM  } //NB         往右變化
                                                                               };   // 8
